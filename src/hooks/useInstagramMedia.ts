...
import { authDevApi, socialDevApi, socialApi } from "@/lib/axios";
...
export function useInstagramMedia() {
  ...
  useEffect(() => {
    async function fetchImportedIds() {
      ...
      try {
        const r = await socialDevApi.get(`/listed-social-posts/${user.id}?skip=0&limit=200`, { headers });
        ids = (Array.isArray(r.data) ? r.data : [])
          .filter((post: any) => post.instagram_id)
          .map((post: any) => post.instagram_id);
      } catch {
        try {
          const r = await socialDevApi.get(`/listed-social-post/${user.id}?skip=0&limit=200`, { headers });
          ids = (Array.isArray(r.data) ? r.data : [])
            .filter((post: any) => post.instagram_id)
            .map((post: any) => post.instagram_id);
        } catch {
  ...
  const fetchMore = useCallback(async () => {
    ...
    try {
      const res = await authDevApi.get(`/instagram/posts?limit=20`, {

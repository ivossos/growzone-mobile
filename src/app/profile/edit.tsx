...
export const EditProfileValidation = z.object({
  ...
  biography: z
    ...
    .max(200, "A biografia pode ter no máximo 200 caracteres"),
  ...
  category_id: z.number({ required_error: 'Campo obrigatório' }).nullable()
});
...
export default function EditProfile() {
  ...
  const handlerOpenButtonSheetProfileCover = () => {
    openBottomSheet({
      type: "profile-cover",
      userId: user.id,
  ...
  const fetchCategories = async () => {
    try {
      ...
      const data = await getUserCategories({ page: 0, size: 30 });
  ...
  async function submit(values: z.infer<typeof EditProfileValidation>) {
    try {
      ...
      router.push({ pathname: "/(drawer)/(tabs)/profile" });
    } catch (err) {
        ...
        >
          ...
          >
            ...
            <View className="relative bg-black-60 mb-10">
              ...
              >
                <TouchableOpacity onPress={handlerOpenButtonSheetProfileCover}>
              ...
              <TouchableOpacity
                ...
                className="flex flex-col justify-center items-center gap-4 px-6 absolute -bottom-14 py-6"
              >
                <Avatar className="relative w-24 h-24 border border-brand-green bg-black-80">
                  {form.watch("image")?.uri ? (
                    <AvatarImage
                      className="rounded-full"
                      source={{ uri: form.watch("image")?.uri }}
                    />
                  ) : (
                    <Fragment>
                      {user.image && (

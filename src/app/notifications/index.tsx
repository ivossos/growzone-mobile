    ...
    try {
      ...
      if (data.length === 0) {
        ...
        setNotifications((prevNotifications) =>
          orderBy(
            uniqBy([...prevNotifications, ...data], "id"),
            "created_at",
            "desc"
          )
        );
      ...
      Toast.show({
        ...
        text1: "Ops!",
        text2:
          "Aconteceu um erro ao buscar as notificações. Tente novamente mais tarde.",
      });
      ...
      <View className="flex flex-row items-center gap-2">
        <Avatar className="w-10 h-10 bg-black-80">
          {item.sender?.image ? (
            <AvatarImage
              ...
              src={item.sender.image.image}
            />
          ) : (
            <AvatarFallback>
              {getInitials(item.sender?.name || item.sender?.username)}
            </AvatarFallback>
          )}
        </Avatar>
        <View className="flex flex-col justify-center gap-2">
          ...
          <View className="flex flex-row items-center gap-1">
            <Text className="text-xs text-brand-grey">

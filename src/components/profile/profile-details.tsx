...
export default function ProfileDetails({ profile, isLoggerUser, isFollow, onUpdateProfile }: ProfileDetailsProps) {
  ...
  const handleOpenRateProfileBottomSheet = () => {
    openBottomSheet({ type: 'rate-profile', userId: profile.info.id, callbackFn: onUpdateProfile});
  };
    ...
    <ScrollView className="flex flex-1 bg-black-100" showsVerticalScrollIndicator={false}>
      ...
      <View style={{ flex: 1 }} className="h-auto">
        <View className="relative bg-black-60">
          ...
          <View className="px-6 absolute -bottom-14">
            <Avatar className="w-28 h-28 border-[4px] border-black-100 bg-black-70">
              { image?.image && <AvatarImage className="rounded-full"   source={{ uri: image.image }} />}
              <AvatarFallback textClassname="text-4xl">{getInitials(info?.name || info?.username)}</AvatarFallback>
            </Avatar>
          </View>
        </View>
        <View className="flex flex-col gap-4 px-6 mt-20">
          ...
          {info?.biography && <View className="flex flex-row items-center gap-1 w-full">
        ...
        <View className="flex flex-col gap-1 px-6 mt-6">
          <View className="flex flex-row justify-between px-6 py-4">
            <TouchableOpacity 
              onPress={() => router.push({ pathname: '/profile/[id]/followers', params: { id: profile.info.id }})} 
              className="flex flex-col items-center gap-1">
              <Text className="text-lg text-white font-medium">{metric.followers}</Text>
              <Text className="text-sm text-brand-grey font-regular">
                Seguidores
              </Text>
            </TouchableOpacity>
            <TouchableOpacity 
              onPress={() => router.push({ pathname: '/profile/[id]/following', params: { id: profile.info.id }})}  
              className="flex flex-col items-center gap-1">
              <Text className="text-lg text-white font-medium">{metric.following}</Text>
              <Text className="text-sm text-brand-grey font-regular">
                Seguindo
              </Text>
            </TouchableOpacity>
            <View className="flex flex-col items-center gap-1">
              <Text className="text-lg text-white font-medium">{formatDateToMonthYear(info.created_at)}</Text>
              <Text className="text-sm text-brand-grey font-regular">
                Membro desde
              </Text>
            </View>
          ...
          <View className="flex flex-row justify-between bg-black-90 rounded-lg px-6 py-4">
            <View className="flex flex-col items-center ">
              <View className="flex flex-row items-center justify-center gap-1">
                ...
                <Text className="text-lg text-brand-white font-medium">20</Text>
              </View>
              <Text className="text-sm text-brand-grey font-regular">
                Posts
              </Text>
            ...
            <View className="flex flex-col items-center gap-1">
              <View className="flex flex-row items-center justify-center gap-1">
                ...
                <Text className="text-lg text-brand-white font-medium">8</Text>
              </View>
              <Text className="text-sm text-brand-grey font-regular">
                Wells
              </Text>
            </View>
            <TouchableOpacity className="flex flex-col items-center gap-1" onPress={handleOpenReviewsProfileBottomSheet}>
              <View className="flex flex-row items-center justify-center gap-1">
                <Ionicons name="star" size={14} color={colors.brand.green} />
                <Text className="text-lg text-brand-white font-medium">{metric.average_review}</Text>
              </View>
              <Text className="text-sm text-brand-grey font-regular">
                Avaliação
              </Text>
          ...
          {!isLoggerUser && <View className="flex flex-row gap-2 mt-6">
            {!!follow ? (
              <TouchableOpacity 

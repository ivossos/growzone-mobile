...
const DeletePostBottomSheet = React.forwardRef<BottomSheet, Props>(({ onClose }, ref) => {
  ...
  const renderBackdrop = useCallback(
    (props: any) => (
      <BottomSheetBackdrop {...props} opacity={0.8} disappearsOnIndex={-1}  appearsOnIndex={0} />
    ),
    []
  );
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["profile", user.id.toString()] });
      queryClient.invalidateQueries({ queryKey: ["profile-posts", user.id.toString()] });
      queryClient.invalidateQueries({ queryKey: ["profile-post-grow", user.id.toString()] });
      queryClient.invalidateQueries({ queryKey: ["profile-reels", user.id.toString()] });
  ...
  const ReportedSuccessfully = () => (
    <BottomSheetView className='flex flex-col items-center flex-1 gap-14 p-6 bg-black-100'>
      <AnimatedSuccess />
      <Text className="text-3xl text-white text-center font-semibold">Seu post foi excluido com sucesso</Text>
      <Text className="text-base font-medium text-center text-brand-grey max-w-80">Aproveite para postar novas atualizações para comunidade</Text>  
  ...
  const renderDecision = () => (
    <BottomSheetView className="flex flex-col flex-1 gap-6 p-6 bg-black-100">
      <BottomSheetView className="flex flex-col items-center gap-2 ">
          <Text className="text-3xl text-white text-center font-semibold">Excluir postagem</Text>
          <Text className="text-base font-medium text-center text-brand-grey max-w-80">Você realmente dejesa excluir seu post?</Text>  
        </BottomSheetView>

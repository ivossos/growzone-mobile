...
const PostBottomSheet = React.forwardRef<BottomSheet, Props>(({ onClose }, ref) => {

  const snapPoints = useMemo(() => ['40%'], []);
  const {  postId, isVisible, currentType, closeBottomSheet, openBottomSheet } = useBottomSheetContext();
  ...
  const navigate = () => {
    ...
    if (currentType && routesMap[currentType]) {
      router.push({ pathname: routesMap[currentType], params: { id: postId } });
    }
  };
    ...
    <BottomSheet
      ...
      >
          ...
          <BottomSheetView className="flex flex-col flex-1 gap-2 p-6 bg-black-100">
            {[
              {
                Icon: <Pencil width={24} height={24} color={colors.brand.green} />,
                title: "Editar post",
                description: "Editar a descrição do seu post",
                onAction: navigate
              },
              {
                Icon: <Trash2 width={24} height={24} color={colors.brand.error} />,
                title: "Excluir post",

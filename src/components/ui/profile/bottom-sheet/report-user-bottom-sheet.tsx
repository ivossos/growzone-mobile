...
const ReportUserBottomSheet = React.forwardRef<BottomSheet, Props>(({ onClose }, ref) => {
  ...
  const { userId, isVisible, currentType, closeBottomSheet } = useBottomSheetContext();
  ...
  const fetchReportReasons = useCallback(async () => {
    try {
      ...
      Toast.show({
        type: 'error',
        text1: 'Opss',
        text2: 'Aconteceu um erro ao buscar as razões de report, tente novamente mais tarde.',
      });
    } finally {
  ...
  const ReportedSuccessfully = () => (
    <BottomSheetView className='flex flex-col items-center flex-1 gap-14 p-6 bg-black-100'>
      <AnimatedSuccess />
      <Text className="text-3xl text-white text-center font-semibold">Sua denúncia foi relizada com Sucesso!</Text>
      <Text className="text-base font-medium text-center text-brand-grey mx-auto">Obrigado! Seu reporte sera revisado pela nossa equipe.</Text>  
  ...
  const ReportedDecision = () => (
    <BottomSheetView className='flex flex-col items-center flex-1 gap-4 p-6 bg-black-100'>
      <Text className="text-3xl text-white text-center font-semibold">Tem certeza que deseja reportar esse perfil?</Text>
      <Text className="text-base font-medium text-center text-brand-grey max-w-60">Seu reporte será enviado e revisado pela nossa equipe.</Text>  
      ...
      >
        <Text className="text-white text-base font-medium">Ainda não me decidi</Text>
      </TouchableOpacity>
    </BottomSheetView>
  );
  ...
  const renderReportOptions = () => (
    <BottomSheetView className="flex flex-col flex-1 gap-6 p-6 bg-black-100">
      <BottomSheetView className='flex flex-col items-center'>
        <Text className="text-3xl text-white text-center font-semibold">Denunciar este perfil</Text>
        <Text className="text-base font-medium text-center text-brand-grey max-w-60">Selecione o motivo para você estar denunciando esse perfil</Text>  
      </BottomSheetView>
  ...
  return (
    <BottomSheet
      ...
      onClose={handleClose}
    >
      {reportSubmitted ? <ReportedSuccessfully /> : isDecided ? <ReportedDecision /> : renderReportOptions()}
    </BottomSheet>
  );
})
